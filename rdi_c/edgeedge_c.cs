using System;
using System.IO;
using system.block.uda.dataSource;

namespace app_c.mma.rdi {
	[Serializable]
	public class edgeedge_c : app_c.mma.rdi.RDISplImplRWM_c {
		private static edgeedge_c thisItem;

		private edgeedge_c(Type oUserType) : base(oUserType) {
		}

		private edgeedge_c() : base(typeof(app.mma.rdi.edgeedge)) {
		}

		public static system.block.uda.dataSource.RawDataItemDB_c getInstance() {
			if (thisItem == null)
				thisItem = new edgeedge_c();

			return thisItem;
		}

		public static system.block.uda.dataSource.RawDataItemDB_c getInstance(system.block.item.Context_c oContext) {
			if (thisItem == null) {
				thisItem = new edgeedge_c();
				thisItem.context = oContext;
			}

			return thisItem;
		}

		public static system.block.uda.dataSource.RawDataItemDB_c getInstance(Type oUserType) {
			if (thisItem == null)
				thisItem = new edgeedge_c(oUserType);

			return thisItem;
		}
	}
}
